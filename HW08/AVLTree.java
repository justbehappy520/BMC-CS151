...
public class AVLTree<E extends Comparable<E>> extends LinkedBinaryTree<E> {
    ...
    public Node<E> rebalance(Node<E> n) {
        ...
        // how in the world did I forget that there's a rebalance method?????????????
        int balanceFactor = getBalance(nnode);
        //System.out.println("befBF: " + balanceFactor);
        //System.out.println("par: " + nnode.getParent());
        while (Math.abs(balanceFactor) < 2 && nnode.getParent() != null) {
            //System.out.println("bef: " + nnode);
            nnode = nnode.getParent();
            //System.out.println("aft: " + nnode);
            balanceFactor = getBalance(nnode);
        }
        //System.out.println("aftBF: " + balanceFactor);
        ...
        if (balanceFactor > 1) {
            if (nnode.getLeft() != null) {
                ...
                int lbf = getBalance(nnode.getLeft());
                //if (lbf > 0) {
                if (lbf >= 0) {
                    System.out.println(":$");
                    //rotateRight(nnode.getLeft());
                    rotateRight(nnode);
                }
                // left-right --> 
                // rotate left on left child, rotate right
                else {
                    System.out.println(":P");
                    nnode.setLeft(rotateLeft(nnode.getLeft()));
                    rotateRight(nnode);
                    //rotateRight(nnode.getLeft());
                    //rotateRight(nnode.getParent());
                }
            }
        }
        // right heavy
        if (balanceFactor < -1) {
            if (nnode.getRight() != null) {
                ...
                int rbf = getBalance(nnode.getRight());
                //if (rbf < 0) {
                if (rbf <= 0) {
                    System.out.println(":$");
                    //rotateLeft(nnode.getRight());
                    rotateLeft(nnode);
                }
                // right-left -->
                // rotate right on right child, rotate left
                else {
                    System.out.println(":P");
                    nnode.setRight(rotateRight(nnode.getRight()));
                    rotateLeft(nnode);
                    //rotateLeft(nnode.getRight());
                    //rotateLeft(nnode.getParent());
                }
            }
        }
        return nnode; // no rebalancing needed
    }
    ...
    private void insertRecursive(Node<E> node, E element) {        
        ...
        //System.out.println("^^" + nnode.getParent());
        //System.out.println("inserted: " + nnode);
	    // nnode is the current root of the tree because reasons beyond me
        // I fiddled with the LLbt so hopefully it returns the inserted node
